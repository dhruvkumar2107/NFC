...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  const nav = [
    ...
    { href: '/admin/employees', label: 'Employees', icon: <svg className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth={1.5} viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M15 19.128a9.38 9.38 0 002.625.372 9.337 9.337 0 004.121-.952 4.125 4.125 0 00-7.533-2.493M15 19.128v-.003c0-1.113-.285-2.16-.786-3.07M15 19.128v.106A12.318 12.318 0 018.624 21c-2.331 0-4.512-.645-6.374-1.766l-.001-.109a6.375 6.375 0 0111.964-3.07M12 6.375a3.375 3.375 0 11-6.75 0 3.375 3.375 0 016.75 0zm8.25 2.25a2.625 2.625 0 11-5.25 0 2.625 2.625 0 015.25 0z" /></svg> },
    ...
    { href: '/admin/orders', label: 'Orders', icon: <svg className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth={1.5} viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M15.75 10.5V6a3.75 3.75 0 10-7.5 0v4.5m11.356-1.993l1.263 12c.07.665-.45 1.243-1.119 1.243H4.25a1.125 1.125 0 01-1.12-1.243l1.264-12A1.125 1.125 0 015.513 7.5h12.974c.576 0 1.059.435 1.119 1.007zM8.625 10.5a.375.375 0 11-.75 0 .375.375 0 01.75 0zm7.5 0a.375.375 0 11-.75 0 .375.375 0 01.75 0z" /></svg> },
    { href: '/admin/orders/new', label: 'New Order', icon: <svg className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth={1.5} viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M12 9v6m3-3H9m12 0a9 9 0 11-18 0 9 9 0 0118 0z" /></svg> },
    { href: '/admin/cards', label: 'Cards', icon: <svg className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth={1.5} viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M2.25 8.25h19.5M2.25 9h19.5m-16.5 5.25h6m-6 2.25h3m-3.75 3h15a2.25 2.25 0 002.25-2.25V6.75A2.25 2.25 0 0019.5 4.5h-15a2.25 2.25 0 00-2.25 2.25v10.5A2.25 2.25 0 004.5 19.5z" /></svg> },
    { href: '/admin/wallet', label: 'Wallet Txns', icon: <svg className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth={1.5} viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M2.25 18.75a60.07 60.07 0 0115.797 2.101c.727.198 1.453-.342 1.453-1.096V18.75M3.75 4.5v.75A.75.75 0 013 6h-.75m0 0v-.375c0-.621.504-1.125 1.125-1.125H20.25M2.25 6v9m18-10.5v.75c0 .414.336.75.75.75h.75m-1.5-1.5h.375c.621 0 1.125.504 1.125 1.125v9.75c0 .621-.504 1.125-1.125 1.125h-.375m1.5-1.5H21a.75.75 0 00-.75.75v.75m0 0H3.75m0 0h-.375a1.125 1.125 0 01-1.125-1.125V15m1.5 1.5v-.75A.75.75 0 003 15h-.75M15 10.5a3 3 0 11-6 0 3 3 0 016 0zm3 0h.008v.008H18V10.5zm-12 0h.008v.008H6V10.5z" /></svg> },
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 via-white to-gray-50/80">
      <aside className="fixed inset-y-0 left-0 w-64 glass-dark-strong z-30 hidden lg:flex flex-col shadow-2xl shadow-black/20 border-r border-white/[0.06]">
        <Link href="/admin/dashboard" className="flex items-center gap-2.5 px-5 py-5 text-lg font-bold text-white">
        ...
        <div className="text-[11px] font-semibold uppercase tracking-wider text-white/40 px-5 mt-4 mb-2">Admin Panel</div>
        <nav className="flex-1 px-3 space-y-0.5 overflow-y-auto max-h-[calc(100vh-12rem)]">
          {nav.map((item) => {
            const active = pathname === item.href
            return (
              <Link key={item.href} href={item.href}
                className={`flex items-center gap-3 px-3 py-2 rounded-xl text-sm font-medium transition-all duration-200 ${active ? 'bg-white/[0.07] text-white border-l-2 border-primary-500' : 'text-white/50 hover:text-white hover:bg-white/[0.04]'}`}>
                {item.icon}
                {item.label}
              </Link>
            )
      ...
      {mobileMenuOpen && (
        <div className="fixed inset-0 z-40 lg:hidden">
          ...
          <div className="absolute inset-y-0 left-0 w-72 glass-dark-strong z-50 flex flex-col shadow-2xl max-h-screen overflow-y-auto">
            <Link href="/admin/dashboard" className="flex items-center gap-2.5 px-5 py-5 text-lg font-bold text-white border-b border-white/[0.06]">
              <div className="w-8 h-8 rounded-xl bg-primary-500/20 flex items-center justify-center"><span className="text-primary-400 text-sm font-bold">M</span></div>
              MySmartCard
            </Link>
            <div className="text-[11px] font-semibold uppercase tracking-wider text-white/40 px-5 mt-4 mb-2">Admin Panel</div>
              ...
              {nav.map((item) => {
                ...
                return (
                  <Link key={item.href} href={item.href} onClick={() => setMobileMenuOpen(false)}
                    className={`flex items-center gap-3 px-3 py-2 rounded-xl text-sm font-medium transition-all ${active ? 'bg-white/[0.07] text-white border-l-2 border-primary-500' : 'text-white/50 hover:text-white hover:bg-white/[0.04]'}`}>
                    {item.icon}
                    {item.label}
                  </Link>
                )
      ...
      <div className="lg:ml-64">
        <header className="lg:hidden glass sticky top-0 z-30 px-4 py-3 flex items-center justify-between border-b border-gray-100/50">
